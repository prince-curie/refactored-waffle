...
const Header = ({
    ...
    try {
      if(web3ModalRef) {
        ...
        if(network.name === process.env.NEXT_PUBLIC_NETWORK) {
          ...
          await getOwner(provider)
  
          toast.success("Wallet connected")
        } else {
          toast.error("Set your network to the goerli test network")
        }
      }
    } catch (error:any) {
  ...
  const getOwner = async(provider: ethers.providers.Web3Provider) => {
    if(web3ModalRef) {
        ...
        {!isWalletConnected ? 
            ...
            <div className='flex sm:inline-flex justify-center sm:justify-between w-full sm:w-fit items-center flex-wrap mt-2 sm:mt-0'> 
              { (isWalletConnected && walletAddress == owner) && <Link href="/dashboard">
                  <a className='text-xl mt-2 sm:mr-3 w-full sm:w-fit font-bold'>dashboard</a>
                </Link>
              }
